...
class P2PEditor:
    def __init__(self, method_list, device, num_ddim_steps=50) -> None:
        ...
        self.ldm_stable = StableDiffusionPipeline.from_pretrained(
            "/data/pretrained_models/SD1.5/", scheduler=self.scheduler).to(device)
    ...
    def __call__(self, 
        ...
        if edit_method=="ddim+p2p":
            ...
            return self.edit_image_directinversion_add_source(image_path=image_path, prompt_src=prompt_src, prompt_tar=prompt_tar, guidance_scale=guidance_scale, 
                                        cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps, 
                                        blend_word=blend_word, eq_params=eq_params, is_replace_controller=is_replace_controller)
        elif edit_method=="freeinv+p2p":
            return self.edit_image_freeinv(image_path, prompt_src, prompt_tar, guidance_scale=guidance_scale, 
                                        cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps, 
                                        blend_word=blend_word, eq_params=eq_params, is_replace_controller=is_replace_controller)
        else:
    ...
    def edit_image_freeinv(
        ...
        angles = [angle_lst[torch.randint(0, 4, (1,))] if i%3==0 else 0 for i in range(self.num_ddim_steps)]

        _, _, x_stars_aug, uncond_embeddings = null_inversion.invert(
            image_gt=image_gt, prompt=prompt_src,guidance_scale=guidance_scale,num_inner_steps=0, angles=angles)
        x_t_aug = x_stars_aug[-1]
        ...
        controller = make_controller(pipeline=self.ldm_stable,
                                    ...
                                    num_ddim_steps=self.num_ddim_steps,
                                    device=self.device)
        # no_rot = int(0.55 * self.num_ddim_steps)
        latents, _ = p2p_guidance_forward(model=self.ldm_stable, 
                                       prompt=prompts, 
                                       controller=controller, 
                                       latent=torch.cat([x_t_aug, x_t], 0), 
                                       num_inference_steps=self.num_ddim_steps, 
                                       guidance_scale=guidance_scale, 
                                       generator=None, 
                                       uncond_embeddings=uncond_embeddings,
