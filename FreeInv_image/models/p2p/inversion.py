...
class NullInversion:
    ...
    def get_noise_pred_single(self, latents, t, context, angle=0):
        latents = TF.rotate(latents, angle)
        noise_pred = self.model.unet(latents, t, encoder_hidden_states=context)["sample"]
        noise_pred = TF.rotate(noise_pred, -angle)
        return noise_pred
    ...
    @torch.no_grad()
    def ddim_loop(self, latent, angles=None):
        uncond_embeddings, cond_embeddings = self.context.chunk(2)
        all_latent = [latent]
        latent = latent.clone().detach()
        angles = angles if angles is not None else [0] * self.num_ddim_steps
        for i in range(self.num_ddim_steps):
            t = self.model.scheduler.timesteps[len(self.model.scheduler.timesteps) - i - 1]
            noise_pred = self.get_noise_pred_single(latent, t, cond_embeddings, angle=angles[i])
            latent = self.next_step(noise_pred, t, latent)
            all_latent.append(latent)
        return all_latent
    ...
    @torch.no_grad()
    def ddim_inversion(self, image, angles=None):
        latent = image2latent(self.model.vae, image)
        image_rec = latent2image(self.model.vae, latent)[0]
        ddim_latents = self.ddim_loop(latent, angles=angles)
    ...
    def invert(self, image_gt, prompt, guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5, angles=None):
        self.init_prompt(prompt)
        register_attention_control(self.model, None)
        
        image_rec, ddim_latents = self.ddim_inversion(image_gt, angles=angles)
