...
class StableDiffusionPipelineOutput(BaseOutput):
    ...
    recon_images: Union[List[PIL.Image.Image], np.ndarray]
...
class EditPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        if not output_type == "latent":
            ...
            recon_image = self.vae.decode(clean_latents / self.vae.config.scaling_factor, return_dict=False)[0]
        else:
            image = pred_x0
            has_nsfw_concept = None
        ...
        recon_image = self.image_processor.postprocess(recon_image, output_type=output_type, do_denormalize=do_denormalize)

        if not return_dict:
            return (image, has_nsfw_concept)

        return StableDiffusionPipelineOutput(images=image, recon_images=recon_image, nsfw_content_detected=has_nsfw_concept)