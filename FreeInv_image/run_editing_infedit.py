...
from models.infedit.pipeline_ead import EditPipeline
import models.infedit.ptp_utils as ptp_utils
import models.infedit.seq_aligner as seq_aligner
import math
import torch
...
model_id_or_path = "/data/pretrained_models/SimianLuo/LCM_Dreamshaper_v7"
...
def edit_image_infedit(source_prompt, target_prompt, positive_prompt, negative_prompt, local, mutual, guidance_s, guidance_t, num_inference_steps=10,
    ...
    img_gt = img
    img_recon = results.recon_images[0]
    img_edit = results.images[0]

    return Image.fromarray(np.concatenate((
        image_instruct,
        np.array(img_gt),
        np.uint8(np.array(img_recon)),
        np.uint8(np.array(img_edit)),
        ),1))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--edit_method_list', nargs = '+', type=str, default=["infedit"]) # the editing methods that needed to run
    ...
    for key, item in editing_instruction.items():
        ...
        for edit_method in edit_method_list:
            ...
            if ((not os.path.exists(present_image_save_path)) or rerun_exist_images):
                ...
                if edit_method=="infedit":
                    edited_image = edit_image_infedit(
                        ...
                        img=image_path,
                width=512, height=512, seed=0, strength=1,
