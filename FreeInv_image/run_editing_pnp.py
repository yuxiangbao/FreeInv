...
class Preprocess(nn.Module):
    ...
    @torch.no_grad()
    def ddim_inversion(self, cond, latent, rot=False, flip=False, shuffle=False, ensemble=False):
        latent_list=[latent]
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            for i, t in enumerate(timesteps):
                ...
                if ensemble:
                    ...
                    cond_batch = cond_batch.repeat(4, 1, 1)
                    model_input_ = torch.cat([TF.rotate(latent, angle=angle) for angle in angle_lst])
                    eps_cat = self.unet(model_input_, t, encoder_hidden_states=cond_batch).sample

                    eps_stk = torch.stack([TF.rotate(eps, angle=-angle) for angle, eps in zip(angle_lst, eps_cat.chunk(4))])
                    eps = eps_stk.mean(0)
                elif self.other_latent is not None:
                    cond_batch = cond_batch.repeat(4, 1, 1)
                    latent = torch.cat([latent]+self.other_latent, dim=0)
                    eps_cat = self.unet(latent, t, encoder_hidden_states=cond_batch).sample
                    eps = torch.mean(eps_cat, dim=0, keepdim=True)
                else:
                    if rot:
                        angle_lst = [0, 90, 180, 270]
                        angle = angle_lst[torch.randint(0, 4, (1,))]
                        latent = TF.rotate(latent, angle=angle)
                        self.angle.append(angle)
                    if flip:
                        func = random.choice([lambda x:x, TF.hflip, TF.vflip])
                        latent = func(latent)
                        self.angle.append(func)
                    if shuffle:
                        latent, indices, reverse_indices = patch_shuffle(latent, self.patch_size)
                        self.angle.append(indices)

                    eps = self.unet(latent, t, encoder_hidden_states=cond_batch).sample

                    if rot:
                        latent = TF.rotate(latent, angle=-angle)
                        eps = TF.rotate(eps, angle=-angle)
                    if flip:
                        latent = func(latent)
                        eps = func(eps)
                    if shuffle:
                        latent, _, _ = patch_shuffle(latent, self.patch_size, reverse_indices)
                        eps, _, _ = patch_shuffle(eps, self.patch_size, reverse_indices)

                pred_x0 = (latent - sigma_prev * eps) / mu_prev
                latent = mu * pred_x0 + sigma * eps
                if self.other_latent is not None:
                    self.other_latent = [latent[[i]] for i in range(1,4)]
                    latent = latent[[0]]
                latent_list.append(latent)
        return latent_list

    @torch.no_grad()
    def ddim_sample(self, x, cond, rot=False, flip=False, shuffle=False, ensemble=False):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            for i, t in enumerate(timesteps):
                    ...
                    if ensemble:
                        angle_lst = [0, 90, 180, 270]
                        cond_batch = cond_batch.repeat(4, 1, 1)
                        model_input_ = torch.cat([TF.rotate(x, angle=angle) for angle in angle_lst])
                        eps_cat = self.unet(model_input_, t, encoder_hidden_states=cond_batch).sample
                        ...
                        cond_batch = cond_batch.repeat(4, 1, 1)
                        x = torch.cat([x]+self.other_latent, dim=0)
                        eps_cat = self.unet(x, t, encoder_hidden_states=cond_batch).sample
                        eps = torch.mean(eps_cat, dim=0, keepdim=True)
                    else:
                        if rot:
                            angle = self.angle.pop()
                            x = TF.rotate(x, angle=angle)
                        if flip:
                            func = self.angle.pop()
                            x = func(x)
                        if shuffle:
                            indices = self.angle.pop()
                            x, indices, reverse_indices = patch_shuffle(x, self.patch_size, indices)
                        ...
                        if flip:
                            ...
                            eps = func(eps)
                        if shuffle:
                           x, _, _ = patch_shuffle(x, self.patch_size, reverse_indices) 
                           eps, _, _ = patch_shuffle(eps, self.patch_size, reverse_indices) 

                    pred_x0 = (x - sigma * eps) / mu
                    x = mu_prev * pred_x0 + sigma_prev * eps
                    if self.other_latent is not None:
                        self.other_latent = [x[[i]] for i in range(1,4)]
                        x = x[[0]]
    ...
    def extract_latents(self, num_steps, data_path,
                        inversion_prompt='', rot=False, flip=False, shuffle=False, ensemble=False, other_image_path=None):
        self.scheduler.set_timesteps(num_steps)

        cond = self.get_text_embeds(inversion_prompt, "")[1].unsqueeze(0)
        image = self.load_img(data_path)
        latent = self.encode_imgs(image)
        if other_image_path is not None:
            other_image = [self.load_img(path) for path in other_image_path]
            self.other_latent = [self.encode_imgs(img) for img in other_image]
        else:
            self.other_latent = None

        inverted_x = self.ddim_inversion(cond, latent, rot=rot, flip=flip, shuffle=shuffle, ensemble=ensemble)
        latent_reconstruction = self.ddim_sample(inverted_x[-1], cond, rot=rot, flip=flip, shuffle=shuffle, ensemble=ensemble)
...
class PNP(nn.Module):
    ...
    def run_pnp(self,image_path,noisy_latent,target_prompt,guidance_scale=7.5,pnp_f_t=0.8,pnp_attn_t=0.5,source_noisy_latent=None):
        ...
        edited_img = self.sample_loop(self.eps,guidance_scale,noisy_latent if source_noisy_latent is None else source_noisy_latent)
...
def edit_image_freeinv_PnP(
    ...
    inverted_x, _, _ = model.extract_latents(data_path=image_path,
                                         ...
                                         inversion_prompt=prompt_src, 
                                         rot=False,)
    source_inverted_x, rgb_reconstruction, _ = model.extract_latents(data_path=image_path,
                                         num_steps=NUM_DDIM_STEPS,
                                         inversion_prompt=prompt_src, 
                                         rot=True,)

    edited_image=pnp.run_pnp(image_path,inverted_x,prompt_tar,guidance_scale, source_noisy_latent=source_inverted_x)
...
def edit_image_ensemble_PnP(
    ...
    inverted_x, _, _ = model.extract_latents(data_path=image_path,
                                         num_steps=NUM_DDIM_STEPS,
                                         inversion_prompt=prompt_src, 
                                         rot=False, ensemble=False, )
    source_inverted_x, rgb_reconstruction, _ = model.extract_latents(data_path=image_path,
                                         num_steps=NUM_DDIM_STEPS,
                                         inversion_prompt=prompt_src, 
                                         rot=False, ensemble=True)
...
def edit_image_multi_branch_PnP(
    image_path,
    prompt_src,
    prompt_tar,
    guidance_scale=7.5,
    ...
    torch.cuda.empty_cache()
    image_gt = load_512(image_path)
    other_image_path = ["/data/code/TokenFlow/data/others/00000.png",
                        "/data/code/TokenFlow/data/others/00001.png",
                        "/data/code/TokenFlow/data/others/00002.png"]
    other_image = [load_512(p) for p in other_image_path]
    inverted_x, _, _ = model.extract_latents(data_path=image_path,
                                         num_steps=NUM_DDIM_STEPS,
                                         inversion_prompt=prompt_src, 
                                         rot=False, ensemble=False)
    source_inverted_x, rgb_reconstruction, _ = model.extract_latents(data_path=image_path,
                                         num_steps=NUM_DDIM_STEPS,
                                         inversion_prompt=prompt_src, 
                                         rot=False, ensemble=False, other_image_path=other_image_path)
...
image_save_paths={
    ...
    "freeinv+pnp": "freeinv+pnp",
    "ablation_flip+pnp": "ablation_flip+pnp",
    "ablation_shuffle+pnp": "ablation_shuffle+pnp",
    "ablation_ensemble+pnp":"ablation_ensemble+pnp",
    "ablation_multi_branch+pnp":"ablation_multi_branch+pnp",
    }
    ...
    for key, item in editing_instruction.items():
        ...
        for edit_method in edit_method_list:
            ...
            if ((not os.path.exists(present_image_save_path)) or rerun_exist_images):
                ...
                elif edit_method=="freeinv+pnp":
                    edited_image = edit_image_freeinv_PnP(
                        image_path=image_path,
                        prompt_src=original_prompt,
                        prompt_tar=editing_prompt,
                        guidance_scale=7.5,
                    )
                elif edit_method=="ablation_flip+pnp":
                    edited_image = edit_image_flip_PnP(
                        image_path=image_path,
                        prompt_src=original_prompt,
                        prompt_tar=editing_prompt,
                        guidance_scale=7.5,
                    )
                elif edit_method=="ablation_shuffle+pnp":
                    edited_image = edit_image_shuffle_PnP(
                        image_path=image_path,
                        prompt_src=original_prompt,
                        prompt_tar=editing_prompt,
                        guidance_scale=7.5,
                    )
                elif edit_method=="ablation_ensemble+pnp":
                    edited_image = edit_image_ensemble_PnP(
                        image_path=image_path,
                        prompt_src=original_prompt,
                        prompt_tar=editing_prompt,
                        guidance_scale=7.5,
                    )
                elif edit_method=="ablation_multi_branch+pnp":
                    edited_image = edit_image_multi_branch_PnP(
                        image_path=image_path,
                        prompt_src=original_prompt,
                        prompt_tar=editing_prompt,
                        guidance_scale=7.5,
