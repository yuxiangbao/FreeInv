...
class MasaCtrlEditor:
    def __init__(self, method_list, device, num_ddim_steps=50) -> None:
        ...
        self.model = MasaCtrlPipeline.from_pretrained(
            "./SD2.1", scheduler=self.scheduler).to(device)
    ...
    def __call__(self, 
        ...
        elif edit_method=="freeinv+masactrl":
            return self.edit_image_freeinv_MasaCtrl(image_path,prompt_src,prompt_tar,guidance_scale,step=step,layper=layper)
        else:
    ...
    def edit_image_freeinv_MasaCtrl(self, image_path,prompt_src,prompt_tar,guidance_scale,step=4,layper=10):
        source_image=load_image(image_path, self.device)
        angle_lst = [0, 90, 180, 270]
        angles = [angle_lst[torch.randint(0, 4, (1,))] for i in range(self.num_ddim_steps)]
        prompts=["", prompt_tar]
        ...
        start_code, latents_list = self.model.invert(source_image,
                                            ...
                                            num_inference_steps=self.num_ddim_steps,
                                            return_intermediates=True,
                                            angles=[0 for _ in range(len(angles))])
        start_code_aug, latents_list_aug = self.model.invert(source_image,
                                                "",
                                                guidance_scale=guidance_scale,
                                            num_inference_steps=self.num_ddim_steps,
                                            return_intermediates=True,
                                            angles=angles)
        start_code_aug = torch.cat([start_code_aug, copy.deepcopy(start_code)], 0)                                     
        # start_code = start_code.expand(len(prompts), -1, -1, -1)
        ...
        no_rot = 20
        image_masactrl = self.model(prompts,
                            latents=start_code_aug,
                            guidance_scale=guidance_scale,
                            angles=[0]*no_rot+angles[no_rot:])
...
image_save_paths={
    ...
    "freeinv+masactrl": "freeinv+masactrl",
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--edit_method_list', nargs = '+', type=str, default=["ddim+masactrl","directinversion+masactrl", "freeinv+masactrl"]) # the editing methods that needed to run
